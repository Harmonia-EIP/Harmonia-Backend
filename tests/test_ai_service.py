...
def make_response(
    ok=True, status_code=200, json_data=None, json_raises=False, text="ERR"
):
    r = MagicMock()
    r.ok = ok
...
def test_ai_url_missing(monkeypatch):
    ...
    with pytest.raises(NoUrlForAIConfiguredException):
        service.call_ai_and_get_patch("hello", TEST_MODEL_ID, TEST_MODEL_NAME)
...
@patch("services.ai_service.requests.post")
def test_ai_success_returns_valid_schema(mock_post, monkeypatch):
    monkeypatch.setattr("services.ai_service.AI_URL", "http://fake.local")
    ...
    mock_post.return_value = make_response(ok=True, json_data=fake_response)

    service = AiService(make_db())

    result = service.call_ai_and_get_patch("hello", TEST_MODEL_ID, TEST_MODEL_NAME)
